...
HRESULT CreateSound(const char *file, SoundData *pData, bool loop)
{
	...
	if (ext != NULL) {
		if (memcmp(ext, ".wav", 4) == 0) {
			hr = LoadWav(file, pData);
		}
		else if (memcmp(ext, ".mp3", 4) == 0) {
			hr = LoadMP3(file, pData);
		}
	}
	if (FAILED(hr)) {
		return hr;
	...
	*----------*/
	...
	// バッファー作成
	ZeroMemory(&pData->xa2Buffer, sizeof(XAUDIO2_BUFFER));
	// サウンドデータのバイト数
	pData->xa2Buffer.AudioBytes = pData->bufSize;
	// サウンドデータの先頭アドレス
	pData->xa2Buffer.pAudioData = pData->pBuffer;
	pData->xa2Buffer.Flags = XAUDIO2_END_OF_STREAM;
	pData->xa2Buffer.LoopCount = loop ? -1 : 0;
	pData->pSource->SubmitSourceBuffer(&pData->xa2Buffer);
...
void PauseSound(const SoundData *pData)
{
	...
	if (!StpBgm)//一時停止
	pData->pSource->Stop(0);
	else// 再生
	pData->pSource->Start(0);
...
void PlaySound(const SoundData *pData)
{
	...
	pData->pSource->GetState(&xa2State);
	...
	{// 再生中
		...
		// オーディオバッファの削除
		pData->pSource->FlushSourceBuffers();
	}
